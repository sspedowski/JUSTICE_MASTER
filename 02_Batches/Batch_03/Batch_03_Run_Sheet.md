...
## One‑Command Run Sheet (Wrapper + PDF)

**Purpose.** Ingest your Top‑10 data without Excel, update `99_Master_Index.csv`, and build `02_Batches/Batch_03/MasterFile_Batch_03.pdf`.
...
**What this does:**
1. Creates/activates `.venv` and installs deps (`pandas`, `openpyxl`, `python-docx`,
  `reportlab`).
1. Imports `Top_10_Failures_Marsh_Case_Summary.xlsx` if present; otherwise parses the
  two DOCX sources.
1. Appends normalized rows to `99_Master_Index.csv` (locked header preserved).
1. Builds `02_Batches/Batch_03/MasterFile_Batch_03.pdf` from rows with **Status** set
  to `✅ Include`.
...
**Purpose.** Ingest your Top‑10 data without Excel, update `99_Master_Index.csv`, and
build `02_Batches/Batch_03/MasterFile_Batch_03.pdf`.
...
If the generator prints *"No ✅ Include rows for Batch 03"*, set Include for the current
Batch 03 rows:
