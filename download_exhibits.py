...
def _force_dropbox_direct(url: str) -> str:
    """Ensure Dropbox preview links use dl=1 for direct download."""
...
def _force_onedrive_download(url: str) -> str:
    """Ensure OneDrive/SharePoint file links include download=1."""
    from urllib.parse import urlparse, parse_qsl, urlencode, urlunparse
...
def _normalize_url(u: str) -> str:
    u = _force_dropbox_direct(u)
    u = _force_onedrive_download(u)
    return u


def try_download(sess: requests.Session, url: str, outdir: Path) -> Path:
    # Normalize preview/folder links (Dropbox / OneDrive)
